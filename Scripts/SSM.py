...
class SSM:
    def __init__(self, data:pd.DataFrame, var_e:float=None, var_h:float=None, a_1:float=None, P_1:float=None, q0:list=None):
        ...
        # System Matrices
        self.T_t = np.zeros(self.N)
        self.R_t = np.zeros(self.N)
        self.Q_t = np.zeros(self.N)
        self.Z_t = np.zeros(self.N)
        self.H_t = np.zeros(self.N)
        
        self.alpha_t = np.zeros(self.N)
        self.M_t = np.zeros(self.N)
        self.K_t = np.zeros(self.N)
        self.L_t = np.zeros(self.N)
    ...
    def k_filter(self, y_t: float, a_t: float, P_t: float, H_t: float, Q_t: float, Z_t: np.array, T_t: np.array, R_t=np.array):
        ...
        if np.isnan(y_t):
            ...
            v_t = y_t - Z_t * a_t
            a_tt = a_t + (M_t * v_t) # L2.S39
            P_tt = P_t - (M_t * F_t * M_t.T) # L2.S39
            

        # Prediction Step
        a_t = T_t * a_tt # L2.S39
        P_t = T_t * P_tt * T_t.T + R_t * Q_t * R_t.T # L2.S39
    ...
    def kalman_filter(self, missing:bool=False):
        ...
        for t, y_t in enumerate(self.df['y_t' + m]):
            ...
            a_tt, P_tt, a_t, P_t, v_t, F_t, K_t = self.k_filter(y_t=y_t, a_t=a_t, P_t=P_t,
                                                                 var_e=self.var_e, Q_t=self.var_h)
